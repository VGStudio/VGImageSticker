package com.app.vgs.vgimagesticker.view.cutout;
...
public class DrawView extends View {
    ...
    private Paint pathPaint, mCirclePaint;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (imageBitmap != null) {
            ...
            for (Pair<Pair<Path, Paint>, Bitmap> action : cuts) {
                if (action.first != null && action.first.first != null) {
            ...
            canvas.drawCircle(pathX, pathY, mStrokeWidth/2, mCirclePaint);
        }
    ...
    private void touchUp() {
        ...
        if (currentAction == DrawViewAction.MANUAL_CLEAR) {
            ...
            cuts.push(new Pair<>(new Pair<>(livePath, pathPaint), bmTemp));
            livePath = new Path();
    ...
    public void undo() {
        if (cuts.size() > 0) {
            ...
            if (cut.second != null) {
                ...
                undoneCuts.push(new Pair<>(new Pair<Path, Paint>(null, null), imageBitmap));
    ...
    public void redo() {
        if (undoneCuts.size() > 0) {
            ...
            if (cut.second != null) {
                cuts.push(new Pair<>(new Pair<Path, Paint>(null, null), imageBitmap));
                this.imageBitmap = cut.second;
            } else {
                cuts.push(cut);
            }
    ...
    private void resizeBitmap(int width, int height) {
        if (width > 0 && height > 0 && imageBitmap != null) {
            ...
            pathX = imageBitmap.getWidth()/2;
            pathY = imageBitmap.getHeight()/2;
    ...
    public void setStrokeWidth(int strokeWidth) {
        ...
        pathPaint.setStrokeWidth(mStrokeWidth);
    }
    ...
    private static class AutomaticPixelClearingTask extends AsyncTask<Integer, Void, Bitmap> {
        ...
        protected void onPreExecute() {
            ...
            drawViewWeakReference.get().cuts.push(new Pair<>(new Pair<Path, Paint>(null, null), drawViewWeakReference.get().imageBitmap));
        }
